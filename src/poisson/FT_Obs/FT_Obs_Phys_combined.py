...
def combined_poisson(repeat, ls_test, num_train, lr=1e-3, pde_weight=1.0, data_weight=0.3,
                     model_restore_path="model/model", trainable_branch=True, trainable_trunk=[True, True, True]):
    import deepxde as dde

    def pde(x, y):
        # Poisson: - (u_xx + u_yy) = f. We set f=0 as placeholder; adjust if forcing known.
...
if __name__ == "__main__":
    ...
    for repeat in range(100):
        output = apply(combined_poisson, (repeat, ls_test, num_train))
        output_list.append(output)
    output_list = np.concatenate(output_list, axis=0)
    np.save(f"predict_ft_obs_phys_combined_poisson.npy", output_list)
