...
def combined_cavity_v(repeat, num_train, lr=5e-4, pde_weight=0.0, data_weight=0.3, model_restore_path="model_v/model"):
...
if __name__ == "__main__":
    ...
    for repeat in range(100):
        output = apply(combined_cavity_v, [repeat, num_train])
        output_list.append(output)
    output_list = np.concatenate(output_list, axis=0)
    np.save(f"predict_ft_obs_phys_combined_cavity_v.npy", output_list)
